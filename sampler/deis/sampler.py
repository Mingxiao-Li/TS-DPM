...
def get_sampler(sde, eps_fn, ts_phase, ts_order, num_step, method="rho_rk",ab_order=2, rk_method="3kutta"):
...
def get_sampler_t_ab(sde, eps_fn, ts_phase, ts_order, num_step, ab_order):
    ...
    def sampler(xT, cut_off_value=None,window_size=None,shift_time_step=False):
        ...
        def ab_body_fn(i, val, cut_off_value=cut_off_value, window_size=window_size,shift_time_step=shift_time_step):
            x, eps_pred = val
            s_t= rev_ts[i]
            new_eps = eps_fn(x, s_t, cut_off_value=cut_off_value, window_size=window_size, shift_time_step=shift_time_step)
